...
def main():
    ...
    if args.prompt:
     # 1. Encode the prompt (e.g., "<user>... <assistant>")
        input_ids = torch.tensor([tokenizer.encode(args.prompt, add_special_tokens=False)], dtype=torch.long).to(device)
        
        # 2. Safety Truncate (keep last 100 tokens to leave room for answer)
        ...
        with torch.no_grad():
            output_ids = model.generate(
                input_ids=input_ids,
                max_length=input_ids.shape[1] + 25, # Short answers are better for logic
                temperature=0.2, # Lower temperature is more "factual"
                top_p=args.top_p,
            )
        
        # 4. Decode ONLY the new tokens
        response_ids = output_ids[0][input_ids.shape[1]:]
        response_text = tokenizer.decode(response_ids.tolist(), remove_special_tokens=False)
        
        # 5. Print a clean marker for the evaluation script to find
        print(f"Generated: {response_text.strip()}")
