...
# Create a small helper to format local JSONL into the HF format your class expects
def load_local_math(path):
    math_samples = []
    with open(path, 'r') as f:
        for line in f:
            # Your math_patch.jsonl has {"text": "..."}
            # We convert it to match the structure: {"conversation": [{"text": "..."}]}
            entry = json.loads(line)
            # Split the text by the tokens to simulate the 'conversation' structure
            # This ensures it passes through your existing __getitem__ perfectly
            math_samples.append({
                "conversation": [
                    {"text": entry['text'].split("<user>")[1].split("<assistant>")[0].strip()},
                    {"text": entry['text'].split("<assistant>")[1].split("<|endoftext|>")[0].strip()}
                ]
            })
    return math_samples
...
def get_hybrid_dataset(hf_dataset_name, math_path, split="train", hf_samples=1):
    # 1. Load the dataset dictionary
    ds_dict = load_dataset(hf_dataset_name)
    
    # 2. Resolve the split name (TinyStories uses 'train' and 'valid')
    actual_split = split
    if split == "validation" or split == "valid":
        actual_split = "valid" if "valid" in ds_dict else "validation"
    
    hf_data = ds_dict[actual_split]
    
    # 3. Load and format local Math Patch
    math_data = load_local_math(math_path)
    
    # 4. Create the mix
    combined_data = []
    hf_subset = hf_data.shuffle(seed=42).select(range(min(hf_samples, len(hf_data))))
    
    for item in hf_subset:
        combined_data.append(item)
    for item in math_data:
        # For validation, we might only want a subset of math to keep it fast
        combined_data.append(item)

    random.shuffle(combined_data)
    
    # We return a dict with a consistent key so the Dataset class finds it
...
def train_and_evaluate(args):
    ...
    # Check if we are doing a Phase 3 "Math Patch"
    math_patch_path = "data/math_patch.jsonl"
    
    if os.path.exists(math_patch_path):
        print(f"--- Phase 3 Detected: Mixing HF data with {math_patch_path} ---")
        # Using 1000 for train and 100 for valid to keep the Math signal strong
        train_data_dict = get_hybrid_dataset(args.dataset, math_patch_path, split="train", hf_samples=1)
        val_data_dict = get_hybrid_dataset(args.dataset, math_patch_path, split="validation", hf_samples=1)
    else:
        print(f"--- Standard Training: Loading {args.dataset} from HuggingFace ---")
        dataset = load_dataset(args.dataset)
        train_data_dict = {"data": dataset["train"]}
        val_data_dict = {"data": dataset["valid"] if "valid" in dataset else dataset["validation"]}

    # Create train and validation datasets - Point split to "data"
    train_dataset = TinyStoriesConversationDataset(
        train_data_dict, tokenizer, max_length=args.max_seq_len, 
        split="data", max_samples=args.max_train_samples,
        user_token=args.user_token, assistant_token=args.assistant_token
    )
    
    val_dataset = TinyStoriesConversationDataset(
        val_data_dict, tokenizer, max_length=args.max_seq_len, 
        split="data", max_samples=args.max_eval_samples,
        user_token=args.user_token, assistant_token=args.assistant_token
    )
    # Create data loaders
